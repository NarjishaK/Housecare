...
const SplitedDetails = () => {
  ...
  const charityName = charitydetails?.charity // Assuming the charity name is stored under this key

  useEffect(() => {
    const fetchSplits = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/splits")
        // Filter the splits based on the charity name
  ...
  // Export to Excel
//   const exportToExcel = () => {
//     const worksheet = XLSX.utils.json_to_sheet(splits.map(split => ({
//       Date: split.date,
//       Id: split._id,
...
//     XLSX.writeFile(workbook, "splits.xlsx")
//   }
